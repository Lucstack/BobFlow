...
const PlatformNode = ({ data, selected, id }) => {
  ...
  const platformOptionsList =
    marketplaceCategories[data.platformType]?.integrations || [];
  const currentPlatform =
    platformOptionsList.find(p => p.id === data.selectedPlatform) ||
    platformOptionsList[0];
        ...
        if (node.id === id) {
          return {
            ...
            data: {
              ...
              integrationDetails: platform.integrationDetails,
            },
          };
        }
        return node;
  ...
  return (
    <div
      className={`custom-node platform-node ${data.category} ${
        selected ? 'selected' : ''
      } ${isMarketplacePartner ? 'marketplace-partner' : ''}`}
    >
                ...
                {platformOptionsList.map(platform => (
                  <div
                    ...
                    className={`platform-option ${
                      platform.id === data.selectedPlatform ? 'selected' : ''
                    }`}
                  ...
                  >
                    ...
                    {platform.isMarketplacePartner && (
                      <span
                        className="marketplace-indicator"
                        title="HiBob Marketplace Partner"
                      >
                        ✓
                      </span>
                    )}
      ...
      <div className="info-box used-for">
        ...
        <EditableField
          initialValue={data.usedFor}
          onSave={newValue => handleSave('usedFor', newValue)}
          fieldType="textarea"
        />
      </div>
      <div className="info-box triggers">
        ...
        <EditableField
          initialValue={data.integrationDetails}
          onSave={newValue => handleSave('integrationDetails', newValue)}
          fieldType="textarea"
        />
      </div>
    </div>
